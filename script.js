...
async function donneeAvion(data) {
    ...
    for (let k in tableauDonneesAvion) {
        if (tableauDonneesAvion[k].reference != null && tableauDonneesAvion[k].reference != '') {
            if (/^[A-Z]{4}[0-9][A-Z0-9\s]*[\s]*$/.test(tableauDonneesAvion[k].reference)) {
                const icaoCode = tableauDonneesAvion[k].reference.substring(0, 4)
                tableauDonneesAvion[k].airline = airlines[icaoCode]

            }else if (/^[A-Z]{3}[0-9][A-Z0-9\s]*[\s]*$/.test(tableauDonneesAvion[k].reference)) {
                const icaoCode = tableauDonneesAvion[k].reference.substring(0, 3)
                tableauDonneesAvion[k].airline = airlines[icaoCode]

            } else if (/^[A-Z]{5}[\s]*$/.test(tableauDonneesAvion[k].reference) || (/^N[0-9][A-Z0-9]{4}[\s]*$/.test(tableauDonneesAvion[k].reference))) {
        ...
        for (let i = 0; i < planes.length; i++) {
            if (planes[i].longitude != null && planes[i].latitude != null) {
                ...
                let popup =
                    ...
                    <p>Compagnie/type d'avion : ${airline}</p>
